...
$con = new db();
$sql=$con->connect();
$query = "select id,fullname,username from seller;";
$result=mysql_query($query,$sql);
$data = array();
...
$res->addMessage("get all seller users");
$res->setData($data);
$res->send();
}
  ...
  $res->send();
   ...
   try {
     ...
     $query="insert into seller (fullname,username,password) values ('".$user->getFullname()."','".$user->getUsername()."','".md5($user->getPassword())."');";
  //   echo "SQL Query to execute: $query"; # Debug Message
     ...
     if ($rows==1){
       ...
       $response->addMessage("User seller created successfully");
       $response->send();
       exit;
     }else {
       $response = new Response ();
       ...
       $response->setData($data);
       $response->addMessage("Error while trying create seller user");
       $response->send();
       exit;
     }
   }
