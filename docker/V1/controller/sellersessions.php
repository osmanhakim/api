...
if ($_SERVER['REQUEST_METHOD']=='POST'){
 ...
 try {
   ...
   $query= "select id,username,password from seller where username='".$input->username."' and password='".$hashpassword."';";
  // echo "SQL Query to execute: $query"; # Debug Message
   ...
   $query="insert into sellersessions (userid,accesstoken,accesstokenexpirey) values (".$data['id'].",'".$accesstoken."',date_add(NOW(),INTERVAL ".$accesstokenexpirey." SECOND));";
   ...
   if ($rows==1){
    ...
    // $response->setData($data);
     $response->addMessage("Error while trying login seller user");
     $response->send();
     exit;
   }
 }
